...
function createBip38(req, res, next) {
  account.getBip38Keys(req.params.userid, req.params.bip38).
    then(function(acc){
      var transaction = qreditjs.transaction.createTransaction(req.params.recipientId, req.params.amount, null, "dummy");
      transaction.senderPublicKey = acc.keys.getPublicKeyBuffer().toString("hex");
      delete transaction.signature;
      qreditjs.crypto.sign(transaction, acc.keys);
      transaction.id = qreditjs.crypto.getId(transaction);
      leveldb.
...
function create(req, res, next) {
  ...
  var transaction = qreditjs.transaction.createTransaction(req.params.recipientId, amount, null, req.params.passphrase);
  leveldb.
    setObject(transaction.id, transaction).
    then(function(){
      res.send({
...
function broadcast(req, res, next) {
  if(req.params.transactions){ //old way
    ...
    then(function(transaction){
      ...
      if (!qreditjs.crypto.verify(transaction)) {
        res.send({
          success: false,
