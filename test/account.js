  ...
  describe('/POST account', () => {
    it('it should create an account on mainnet', (done) => {
      ...
      send({
        ...
        res.should.have.status(200);
        res.body.success.should.be.equal(true);
        res.body.account.address.should.be.equal("UdAwTiByRp5BFyGz9uxXuNYa1KGHT4rmt");
    ...
    it('it should create transaction from bip38 backup using userid', (done) => {
      ...
      send({
        ...
        qreditjs.crypto.verify(res.body.transaction).should.be.equal(true);
        done();
      });
    });
